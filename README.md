...
- **2026-09-20（Google Search Console 驗證檔）** — 新增 `public/googlefac0095feeb277cd.html`（內容為 `google-site-verification: googlefac0095feeb277cd.html`），用於在 Search Console 以「網址前置字元 `https://inspection20.vercel.app/`」資源驗證網站擁有權。**此檔案要一直保留**，刪除後驗證會失效。之後若換成自有網域，要在新網域另外驗證（並更新 `config/site.ts` 的 `SITE_URL`）。
