...
- **2026-09-06** — 修正 Supabase Dashboard 的 Authentication → URL Configuration：Site URL 原本被設成當初串接 Vercel 時自動帶入的舊 preview 部署網址（`https://inspection20-leo-8739.vercel.app`），導致密碼重設信/驗證信在 Redirect URL 沒對到允許清單時會被導去這個隨機、可能已失效的網址，而不是正式網域。改回固定填 `https://inspection20.vercel.app`，並確認 Redirect URLs 允許清單已包含 `https://inspection20.vercel.app/*` 與 `/reset-password`（補齊了上一筆帳號自助管理紀錄裡留的待辦）。舊的 `leo-8739` preview 網址規則先留著沒刪，之後確定沒影響再清。
