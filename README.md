...
- **2026-09-13（五）** — 修正（四）之後使用者回報「選單收起來的時候會有點卡」：`menuChromeOpen` 在 `onExitComplete` 那一刻才切回 `rounded-full`／小尺寸，但選單卡片的「高度」變化（因為選單項目那個 `motion.div` 從 DOM 移除）跟「圓角、背景色」的 class 切換是同一瞬間發生、且沒有任何過渡效果，等於收合動畫播完的那一刻畫面直接「跳」一下（高度和圓角同時瞬間改變），這就是卡頓感的來源。把選單外層容器（`Navigation.tsx` 那個套圓角/背景的 `<div>`）改成 `motion.div` 並加上 framer-motion 的 `layout` 動畫（`layout={!reduceMotion}`，`prefers-reduced-motion` 時關閉），讓它偵測到尺寸變化時自動用平滑的 FLIP 動畫過渡（framer-motion 對 `layout` 動畫中的 border-radius 變化有內建的等比例修正，兩個屬性會一起平滑過渡，不會再瞬間跳動）。`npx tsc --noEmit` 通過；這個改動理論上也會讓一般「滾動時導覽列從大尺寸縮成小尺寸」的過渡更平滑，但那部分本來就有獨立的 CSS transition，需要使用者實機確認滾動時沒有出現新的雙重動畫怪異感。
