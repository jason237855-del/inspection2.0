...
- **2026-09-19（導覽列 logo：點擊回到最上方）** — 使用者要求點導覽列的 logo 回到最上面。原本 logo 只是 `<Link to="/">`：在首頁點它不會有任何反應（已在 `/`，`ScrollToTop` 只在路徑改變時才捲動）。`Navigation.tsx` 新增 `handleLogoClick`：在首頁時攔截點擊、平滑捲回頂端（系統設定「減少動態效果」時改為直接跳到頂端）並清掉網址列的 `#區塊`（如 `/#pricing`）、順便收起手機選單；在其他頁面則維持原本行為（導向首頁，由 `ScrollToTop` 捲到頂端）。logo 連結加上 `aria-label="回到首頁最上方"`。
